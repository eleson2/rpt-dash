...
- **Predefined parameterized reports** (Reports tab): server-registered reports with rich,
  ...
  chart for one LPAR over a **datetime** range (minute precision, e.g. `2026-05-01 08:00` →
  `2026-05-03 08:30`) and a chosen time granularity (1 minute → 1 day), where you choose which
  service classes to show individually and the rest are summed into an "Other" band on top.
  Coarser granularities average over the interval (via DuckDB `time_bucket`). Add the sample with the
